...
void HuffmanDecoding(Node* root, int &index, string str)
{
    if (root == nullptr) {
        return;
    }
    ...
    if (str[index] == '0') {
        HuffmanDecoding(root->left, index, str);
    }
    else {
        HuffmanDecoding(root->right, index, str);
    }
}
*/
// Builds Huffman Tree and decodes the given input text
string HuffmanEncoding(const string& text)
{
...
/*
    ...
    if (isLeaf(root))
    {
        ...
        while (index < (int)str.size() - 1) {
            HuffmanDecoding(root, index, str);
        }
    }
*/
